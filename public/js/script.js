$(document).ready(function () {
    ...
    $('.createRupric').click(function () {
        title = $('#inputName').val();
        parentId = $('#rubric-id').val();
        checkedFields = [];
        $('input.field-checkbox[type=checkbox]:checked').each(function(item) {
    ...
    $('body').on('click', '.remove-rubric-btn', function () {
        const id = $(this).data('id');
        url = '/api/rubric/remove';
        params = { id: id };
        function removeResponseFunc(responce) {
            if (responce.data['status'] == 'success') {
                Toast.fire({
    ...
    $("#rubric-list").change(function () {
        id = $(this).val();
        title = $(this).find(`[value=${id}]`).text();
        url = '/api/rubric/getChild';
        params = { id: id };

        function changeListResponse(response) {
            $('#rubric_array').append('<button class="btn btn-info btn-sm mr-2 chips-btn" data-id="' + $('#rubric-list option:selected').val() + '">' + $('#rubric-list option:selected').text() +'</button>');
            $('#rubric-id').val(id);
            if(response.data && response.data.length > 0) {
                $('#rubric-list').children().remove();
                optionStr = '<option value="0">Нет</option>';
                response.data.forEach(function (item) {
                    optionStr = optionStr + '<option value="' + item.id + '">' + item.title + '</option>';
                });
                $('#rubric-list').html(optionStr);
            } else {
                $('#rubric-list').children().remove();
                $('#rubric-list').html('<option value="0">Нет</option>');
    ...
    $('body').on('click', '#rubric_array .chips-btn', function(){
        let block = $(this), count = 0;
        $('#rubric-list').attr('disabled', false);
        $('.chips-btn').each(function (i, f) {
            if($(f).data('id') == block.data('id')) {
                count = i;
            }
